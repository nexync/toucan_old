    ...
    with torch.no_grad():
        for i, (data, target, seq_len, boundaries) in enumerate(eval_iter.get_fixlen_iter()):
            ...
            for j in range(args.batch_chunk):
                ...
                total_loss += n_chars * loss.item()
                total_len += n_chars
                total_tokens += pred_boundaries.long().sum().item()
                #</CHANGE>
            for k, v in stats.items():
                stats_agg[k].append(v)
                
            #<ADDITION>
            stats_agg['chars_per_token'] = total_len / total_tokens
...
def main():
    ...
    #<CHANGE>
    bpt = stats_test['chars_per_token'] * (test_loss / math.log(2))
    print_once('| End of training | test loss {:5.2f} | test bpc {:9.5f} | test bpt {:9.5f}'.format(
        test_loss, test_loss / math.log(2), bpt), args
    )
    #</CHANGE>
